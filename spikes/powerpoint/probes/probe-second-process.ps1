...
function Start-Ppt([string]$Exe, [string]$Desktop, [string]$File) {
    ...
    [pscustomobject]@{ Ok = $ok; Pid = $pi.pid }
}
...
$launched = @()      # pids WE created, and the only ones we may kill
$desk = [IntPtr]::Zero
...
try {
    ...
    if ($alive1) { $launched = $launched | Where-Object { $_ -ne $p1.Pid }; Stop-Process -Id $p1.Pid -Force -ErrorAction SilentlyContinue; Say "      swept $($p1.Pid)"; Start-Sleep -Milliseconds 900 }
    ...
    foreach ($p in ($launched | Sort-Object -Unique)) {
        if (Get-Process -Id $p -ErrorAction SilentlyContinue) {
            Stop-Process -Id $p -Force -ErrorAction SilentlyContinue
            Say ("      swept launched pid {0}" -f $p)
        }
    }
