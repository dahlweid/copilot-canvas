...
| step | scale | viewer | page css | canvas px | scrollW | scrollLeft floor | leftReach | spans | layer w |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| open (fit-width) | 1.394 | 885 | 853 | 853 | 885 | 0 | +16 | 1 | 853 |
| zoom in ×1 | 1.742 | 885 | 1066 | 1066 | 1098 | 0 | +16 | 1 | 1066 |
| zoom in ×2 | 2.178 | 885 | 1332 | 1332 | 1364 | 0 | +16 | 1 | 1332 |
| zoom out ×1 | 1.742 | 885 | 1066 | 1066 | 1098 | 0 | +16 | 1 | 1066 |
| fit-height | 0.891 | 885 | 545 | 545 | 885 | 0 | +170 | 1 | 545 |
| fit-width | 1.394 | 885 | 853 | 853 | 885 | 0 | +16 | 1 | 853 |
| **fitted, panel → 520** | **0.773** | **505** | **473** | 473 | 505 | 0 | +16 | 1 | 473 |
| **zoomed 0.966, panel → 900** | **0.966** | **885** | **591** | 591 | 885 | 0 | +147 | 1 | 591 |
...
that exists and cannot be reached. `canvas px` is `HTMLCanvasElement.width` —
the bitmap's own pixel width, not its CSS width.
...
a wide page grows the scrollable area instead of overflowing it, and the
centring keeps doing its job while the page still fits (rows 1 and 5 above:
+16px and +170px of real margin). Chosen over `align-items: safe center`, which
fixes the same thing but only where the `safe` keyword is supported.
